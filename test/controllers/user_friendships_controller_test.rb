...
class UserFriendshipsControllerTest < ActionController::TestCase
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @friendship1 = create(:pending_user_friendship, user: users(:jake), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @friendship2 = create(:accepted_user_friendship, user: users(:jake), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
  ...
  context "#new" do
  	...
  	context "when logged in" do
  		...
  		should "display the friend's name" do
  			get :new, friend_id: users(:jacob)
  			assert_match /#{users(:jacob).full_name}/, response.body
  		end
  		...
  		should "assign a new user friendship to the correct friend" do
  			get :new, friend_id: users(:jacob)
  			assert_equal users(:jacob), assigns(:user_friendship).friend
  		end

  		should "assign a new user friendship to the currently logged in user" do
  			get :new, friend_id: users(:jacob)
  			assert_equal users(:jake), assigns(:user_friendship).user
  		end
      ...
      should "ask if you really want to friend the user" do
        get :new, friend_id: users(:jacob)
        assert_match /Do you really want to friend #{users(:jacob).full_name}?/, response.body
      end
  	end
  end
  ...
  context "#create" do
    ...
    context "when logged in" do
      ...
      context "with a valid friend_id" do
        ...
        should "assign a user friendhsip object" do
          assert assigns(:user_friendship)
          assert_equal users(:jake), assigns(:user_friendship).user
          assert_equal users(:mike), assigns(:user_friendship).friend
        end

        should "create a frienship" do
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:jake), friend: @friend)
        create(:pending_user_friendship, friend: users(:jake), user: @friend)
        sign_in users(:jake)
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:jake))
        create(:accepted_user_friendship, friend: users(:jake), user: @friend)
        sign_in users(:jake)
      end
