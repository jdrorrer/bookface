...
class UserFriendshipTest < ActiveSupport::TestCase
	...
	test "that creating a frienship works without raising an exception" do
		assert_nothing_raised do
			UserFriendship.create user: users(:jake), friend: users(:mike)
		end
	end

	test "that creating a frienship based on user id and friend id works" do
		UserFriendship.create user_id: users(:jake).id, friend_id: users(:mike).id
		assert users(:jake).pending_friends.include?(users(:mike))
	end

	context "a new instance" do
		setup do
			@user_friendship = UserFriendship.new user: users(:jake), friend: users(:mike)
		end
	...
	context "#send_request_email" do
		setup do
			@user_friendship = UserFriendship.create user: users(:jake), friend: users(:mike)
		end
	...
	context "#mutual_friendship" do
		setup do
			UserFriendship.request users(:jake), users(:jacob)
			@friendship1 = users(:jake).user_friendships.where(friend_id: users(:jacob).id).first
			@friendship2 = users(:jacob).user_friendships.where(friend_id: users(:jake).id).first
		end
	...
	context "#accept!" do
		setup do
			@user_friendship = UserFriendship.request users(:jake), users(:mike)
		end
		...
		should "include the friend in the list of friends" do
			@user_friendship.accept!
			users(:jake).friends.reload
			assert users(:jake).friends.include?(users(:mike))
		end
	...
	context ".request" do
		should "create two user friendships" do
			assert_difference 'UserFriendship.count', 2 do
				UserFriendship.request(users(:jake), users(:mike))
			end
		end
	...
	context "#block!" do
		...
		should "not allow new requests once blocked" do
			@user_friendship.block!
			uf = UserFriendship.request users(:jake), users(:mike)
			assert !uf.save
		end
	end
