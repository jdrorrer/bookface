...
class AddAFriendTest < ActionDispatch::IntegrationTest
  ...
  test "that adding a friend works" do
  	sign_in_as users(:jake), "testing"

  	get "/user_friendships/new?friend_id=#{users(:jacob).profile_name}"
  	assert_response :success

  	assert_difference 'UserFriendship.count', 2 do
  		post "/user_friendships", user_friendship: { friend_id: users(:jacob).profile_name }
